...
{	
	...
	$array = $connection->query("select * from users where id= '$_row[id]' ");
	$row = $array->fetch_assoc();
	if ($process == 'credit') 
	{
		$balance = $row['checking_balance'] + $amount;
		return $connection->query("update users set checking_balance = '$balance' where checking_balance = '$row[checking_balance]'");
	}else
	{
